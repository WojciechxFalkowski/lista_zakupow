...
var safeGlobalThis = check(typeof globalThis == "object" && globalThis) || check(typeof window == "object" && window) || check(typeof self == "object" && self) || check(typeof global == "object" && global) || function() {
  return this;
...
var safeGlobalThis$1 = check$1(typeof globalThis == "object" && globalThis) || check$1(typeof window == "object" && window) || check$1(typeof self == "object" && self) || check$1(typeof global == "object" && global) || function() {
  return this;
}() || Function("return this")();
...
var worker = {
  async fetch(req, env) {
    ...
    if (!token)
      return toError('Missing "authorization" header.', 401);
    try {
      const credentials = Credentials.apiKey(token);
      var user = await App2.logIn(credentials);
      var client = user.mongoClient("mongodb-atlas");
    } catch (err) {
      return toError("Authentication error.", 500);
    }
    const collection = client.db("lista_zakupow").collection("lista");
      ...
      switch (path) {
        case "/api/products":
          if (method === "GET") {
            ...
            console.log("id");
            console.log(id);
            return reply(await collection.deleteOne({ _id: new ObjectId2(id) }));
          } else if (method === "PATCH") {
            const id = url.searchParams.get("id");
            const product = await collection.findOne({ _id: new ObjectId2(id) });
            return reply(await collection.updateOne({ _id: new ObjectId2(id) }, { $set: { purchased: !product.purchased } }));
          }
          break;
        default:
...
function handleCors() {
  const headers = new Headers(handleCorsHeaders);
